...
(function() {
  ...
  class UserChangeInAddress {
    ...
    #config = {
      ...
      buttonClasses: ['btn', 'btn--round', 'btn--warning', 'mt-5', 'mx-auto'],
    ...
    #createForm() {
      ...
      const buttonSubmit =  this.#createElement('div', { className: 'text-center' });
      buttonSubmit.append(this.#createElement('button', { 
    ...
    #createDateField () {
      ...
      const inputGroup = this.#createElement('div', { className: 'input-group custom-select' });
    ...
    #createZipCodeContainer() { 
      ...
      const strikethrough = this.#createElement('span', {
        className: 'd-inline-block strike-through mx-3',
        textContent: '-'
      });
    ...
    #setupEventListeners() { 
      ...
      document.addEventListener('touchstart', (event) => {
        ...
        if (
          ...
          // Nếu cần gửi signal cho Flutter để native cũng đóng bàn phím:
          // window.error?.postMessage(JSON.stringify({
          //   type: 'hideKeyboard'
          // }));
        }
    ...
    postMessage (fncName, msg) {
      try {
        ...
        if (window[fncName]?.postMessage) {
          ...
          if (window.error?.postMessage) {
            window.error.postMessage(JSON.stringify({
              type: 'error',
              code: 'CHANNEL_NOT_FOUND',
              message: `JavaScript channel ${fncName} is not defined`,
              details: {}
            }));
          }
        }
        ...
        if (window.error?.postMessage) {
          window.error.postMessage(JSON.stringify({
            ...
            message: 'Failed to send message to Flutter',
